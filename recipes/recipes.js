...
function renderRecipes(recipeList) {
  ...
  recipeList.forEach(recipe => {
    ...
    const ratingText = `Rating: ${recipe.rating} out of 5 stars`;
    const fullStars = "⭐".repeat(Math.floor(recipe.rating));
    const halfStar = recipe.rating % 1 >= 0.5 ? "⭐" : "";
    const emptyStars = "☆".repeat(5 - Math.ceil(recipe.rating));

    card.innerHTML = `
      <img src="${recipe.image}" alt="${recipe.name}" />
      <h2>${recipe.name}</h2>
      <span class="rating" role="img" aria-label="${ratingText}">
        <span aria-hidden="true">${fullStars}${halfStar}${emptyStars}</span>
      </span>
